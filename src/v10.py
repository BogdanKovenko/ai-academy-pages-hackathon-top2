...
def als_cg(R, factors=128, iterations=20, reg=0.01, alpha=2.0):
    Rc = R.tocsr()
    Rt = R.T.tocsr()
    n_u, n_i = R.shape
...
def train_ease(R, reg=500.0, max_items=15000):
    ...
    if np.count_nonzero(item_counts) > max_items:
        top_idx = np.argsort(-item_counts)[:max_items]
        active_items = np.sort(top_idx)
    else:
...
def build_item2vec(inter_df, window=5, dim=64, label=''):
    ...
    item_counts = inter_df['edition_id'].value_counts()
    freq = set(item_counts[item_counts >= 2].index)
    ...
    for seq in seqs:
        ...
        for i, a in enumerate(f):
            for j in range(max(0, i-window), min(len(f), i+window+1)):
                if i != j:
                    cooc[(a, f[j])] += 1.0 / abs(i - j)
    freq_list = sorted(freq)
    fmap = {iid: i for i, iid in enumerate(freq_list)}
    nf = len(freq_list)
    rs, cs, vs = [], [], []
    for (a, b), v in cooc.items():
        if a in fmap and b in fmap:
            rs.append(fmap[a]); cs.append(fmap[b]); vs.append(v)
    C = sparse.csr_matrix((vs, (rs, cs)), shape=(nf, nf))
    row_s = np.array(C.sum(axis=1)).flatten()
    col_s = np.array(C.sum(axis=0)).flatten()
    total = C.sum()
    Cc = C.tocoo()
    pmi = np.log(Cc.data * total / (row_s[Cc.row] * col_s[Cc.col] + 1e-10) + 1e-10)
...
def build_all_embeddings(inter_df, label=''):
    E = {}
    mat = build_matrix(inter_df)

    mat_log = mat.copy(); mat_log.data = np.log1p(mat_log.data * 2.0)
    _s1 = TruncatedSVD(n_components=64, n_iter=12, random_state=42)
    E['u_svd'] = _s1.fit_transform(mat_log); E['i_svd'] = _s1.components_.T

    mat_bin = mat.copy(); mat_bin.data = np.ones_like(mat_bin.data)
    _s2 = TruncatedSVD(n_components=32, n_iter=10, random_state=123)
    E['u_svd2'] = _s2.fit_transform(mat_bin); E['i_svd2'] = _s2.components_.T

    E['u_als'], E['i_als'] = als_cg(mat, factors=128, iterations=20, reg=0.01, alpha=2.0)

    E['u_als2'], E['i_als2'] = als_cg(mat, factors=64, iterations=15, reg=0.05, alpha=5.0)
    ...
    _sa = TruncatedSVD(n_components=32, n_iter=10, random_state=42)
    E['u_auth'] = _sa.fit_transform(mata); E['a_auth'] = _sa.components_.T
    ...
    _sg = TruncatedSVD(n_components=24, n_iter=10, random_state=42)
    E['u_genre'] = _sg.fit_transform(matg); E['g_genre'] = _sg.components_.T
    ...
    _sp = TruncatedSVD(n_components=16, n_iter=10, random_state=42)
    E['u_pub'] = _sp.fit_transform(matp); E['p_pub'] = _sp.components_.T
    E['p2i'] = p2i_local
...
content_vecs = pca.fit_transform(content_vecs)
norms = np.linalg.norm(content_vecs, axis=1, keepdims=True)
content_vecs = content_vecs / (norms + 1e-9)
...
def build_caches(inter_df, emb_dict, label=''):
    ...
    for uid, grp in df.groupby('user_id'):
        ...
        if vecs:
            ws = np.array(ws)
            p = np.average(vecs, axis=0, weights=ws)
            n = np.linalg.norm(p)
            if n > 0: p /= n
            C['u_i2v'][uid] = p
    ...
    for uid, books in tqdm(C['uhist'].items(), desc=f'Covisit {label}', leave=False):
        ...
        if nn > 1:
            for ai in range(nn):
                for bi in range(nn):
                    if ai != bi:
                        C['co'][(w[ai],w[bi])] += 1.0/(1.0+abs(ai-bi)*0.1)
...
def score_pairs(df, E, C, label=''):
    ...
    for i,(u,e) in enumerate(zip(uids,eids)):
        ...
        if ui is not None and ei is not None:
            svd1[i]=E['u_svd'][ui]@E['i_svd'][ei]
            svd2[i]=E['u_svd2'][ui]@E['i_svd2'][ei]
            als1[i]=E['u_als'][ui]@E['i_als'][ei]
            als2[i]=E['u_als2'][ui]@E['i_als2'][ei]
            bpr1[i]=E['u_bpr'][ui]@E['i_bpr'][ei]
    df['svd_item']=svd1; df['svd_item2']=svd2
    df['als_score']=als1; df['als_score2']=als2; df['bpr_score']=bpr1
    ...
    for i,(u,e) in enumerate(zip(uids,eids)):
        ui = u2i.get(u)
        ei_global = i2i.get(e)
        if ui is None or ei_global is None: continue
        ei_ease = ease_remap.get(ei_global)
        if ei_ease is None: continue
        s, end = ease_R.indptr[ui], ease_R.indptr[ui+1]
        if s < end:
            ease_sc[i] = ease_R.data[s:end] @ ease_B[ease_R.indices[s:end], ei_ease]
    df['ease_score'] = ease_sc

    i2v = E.get('i2v', {})
    i2v_sc = np.zeros(n)
    for i,(u,e) in enumerate(zip(uids,eids)):
        up = C.get('u_i2v', {}).get(u)
        ev = i2v.get(e)
        if up is not None and ev is not None:
            i2v_sc[i] = np.dot(up, ev)
    df['i2v_score'] = i2v_sc
    ...
    for i,(u,e) in enumerate(zip(uids,eids)):
        ...
        if ui is not None and pub is not None:
            pi = E['p2i'].get(pub)
            if pi is not None:
                pub_sc[i] = E['u_pub'][ui] @ E['p_pub'][pi]
    df['svd_pub'] = pub_sc
    ...
    for i,(u,e) in enumerate(zip(uids,eids)):
        ...
        evs=E['i_svd'][ei]; ens=np.linalg.norm(evs)
        eva=E['i_als'][ei]; ena=np.linalg.norm(eva)
    ...
    df['icf_svd_avg']=ics_a; df['icf_svd_max']=ics_m
    df['icf_als_avg']=ica_a; df['icf_als_max']=ica_m
    ...
    for i,(u,e) in enumerate(zip(uids,eids)):
        ...
        for nid,sim in nbs:
            if nid in C['hsets'] and e in C['hsets'][nid]:
                w = 3.0 if C['hw'].get((nid,e),1)==2 else 1.0
                sc += sim*w
            st += abs(sim)
    ...
    df['auth_compl']=ac; df['auth_u_rat']=aur; df['auth_g_rat']=agr
    df['author_match']=am; df['lang_match']=lm
...
def add_cand_rank_features(df):
    rank_cols = ['svd_item','als_score','bpr_score','knn_score','covisit',
                 'global_pop','content_sim','icf_svd_avg','genre_cosine','svd_author',
                 'ease_score','i2v_score']
...
def build_user_feats(inter_df, C):
    uf = inter_df.groupby('user_id').agg(
        u_total=('event_type','count'), u_reads=('event_type',lambda x:(x==2).sum()),
        u_read_ratio=('event_type',lambda x:(x==2).mean()),
        u_avg_rating=('rating','mean'), u_std_rating=('rating','std'),
        u_unique=('edition_id','nunique'),
        u_last_ts=('event_ts','max'), u_first_ts=('event_ts','min'),
    ).reset_index()
...
def build_edition_feats(inter_df):
    ef = inter_df.groupby('edition_id').agg(
        e_total=('user_id','count'), e_reads=('event_type',lambda x:(x==2).sum()),
        e_wishes=('event_type',lambda x:(x==1).sum()), e_avg_rating=('rating','mean'),
        e_n_ratings=('rating',lambda x:x.notna().sum()), e_n_users=('user_id','nunique'),
    ).reset_index()
...
neg_rows = []; rng = np.random.RandomState(42)
for uid in tqdm(val_pos['user_id'].unique(), desc='Neg', leave=False):
    cs = cand_sets.get(uid, set())
    if not cs: continue
    pos_eds = set(val_pos[val_pos['user_id']==uid]['edition_id'])
    neg_pool = list(cs - pos_eds)
    if not neg_pool: continue
    n_neg = min(len(neg_pool), max(len(pos_eds)*20, 50))
    chosen = rng.choice(neg_pool, size=n_neg, replace=False)
    for e in chosen: neg_rows.append({'user_id':uid, 'edition_id':e, 'target':0})
...
for idx,cfg in enumerate(configs):
    m = CatBoostRanker(
        ...
        l2_leaf_reg=cfg['l2'], verbose=500, random_seed=cfg['seed'],
        loss_function=cfg['loss'], eval_metric='NDCG:top=20', task_type='GPU',
        bagging_temperature=cfg.get('bag_temp',0),
    )
    m.fit(pool)
    models.append(m); gc.collect()

fi = models[0].get_feature_importance(data=pool)
fi_pairs = sorted(zip(feature_cols, fi), key=lambda x:-x[1])[:25]
print('Top-25 features:')
for f,imp in fi_pairs: print(f'    {imp:6.1f}  {f}')
...
def local_validate(scored_df, val_df, k=20):
...
local_score = local_validate(val_cdf, val_inter, k=20)
...
for i, m in enumerate(models):
    p = m.predict(test_df[feature_cols])
    p = (p - p.mean()) / (p.std() + 1e-8)
    test_df['score'] += p * weights[i]

print('Ranking...')
rows = []
for uid in tqdm(targets['user_id'].unique(), desc='Ranking'):
    grp = test_df[test_df['user_id']==uid]
    if len(grp)==0: continue
    top20 = grp.nlargest(20, 'score')
    for rank,(_,row) in enumerate(top20.iterrows(),1):
        rows.append([uid, int(row['edition_id']), rank])
...
fdf.to_csv('submission888.csv', index=False)

print(f'Local val: {local_score:.4f}')
